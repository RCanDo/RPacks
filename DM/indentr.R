...
##                default 0 means that the outermost list's entries ("roots") are printed with no indentation;
##                printing of deeper levels is performed recursively each time with level increased by one.
##                'level' is directly translated to the length of indentation which is ind*level.
...
## indentr() by default does not display attributes
indentr(mod, rm="lm")
...
indent(indentr(mod, rm="lm", vsep=1, info=FALSE, messages=FALSE), ind="#  ")
## compare with
mod
mod[]
...
indentr(mod, rm="lm", omit="terms")            ## only name of mod$terms printed
...
## but it doesn't work until you turn off 'info' of indentr():
