...
## Internally cat() is used for giving output to the screen thus quotations of character values are not printed
...
## ---------------------------------------------------------------------------------------------------------------------—•°
print.compact <- function(x, ind="   ", sep=" ", digits=getOption("digits"), ...){
## ------------------------------------------------------------------------------------------------—•°
...
if(is.atomic(x)){
   ...
   ## printing
   x <- as.character(x)
   if(n>0 & n<length(x)){ cat(x[1:n], "...", sep=sep) }else{ cat(x, sep=sep) }
   ## info
...
## non trivial lists
ll <- list("df"=df1, "empty_list"=l0, aa, "mm"=mm, ff, "txt"=c("qq", "hop sa\nsa", "a\tb", "up\ndown"))
ll
